...
def _posalji_podsjetnik(email, event):
    ...
    try:
        ...
        body = (
            ...
            f"---\n"
            f"LEGAL-SUITE - Automatski podsjetnik"
        )
    ...
    with tab_pregled:
        if not eventi:
            ...
            for i, event in enumerate(eventi):
                ...
                # HTML fragmenti s navodnicima izvuceni iz f-string izraza:
                # obrnute kose crte u f-string izrazu trazile bi Python 3.12
                # (PEP 701), a README obecava 3.10+.
                opis_html = (
                    '<div style="color:#475569;font-size:0.85rem;margin-top:0.3rem;">'
                    + _esc(opis) + "</div>"
                ) if opis else ""
                predmet_html = (
                    '<div style="color:#94A3B8;font-size:0.8rem;margin-top:0.2rem;">Predmet: '
                    + _esc(predmet) + "</div>"
                ) if predmet else ""
                st.markdown(
