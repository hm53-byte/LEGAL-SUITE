...
> **NAPOMENA**: Ovaj dokument je inicijalni nacrt sastavljen kao tehnički predložak. Mora biti **pregledan od strane odvjetnika** prije aktivacije naplate u live mode-u (Polar.sh, odnosno Stripe u naslijeđenoj integraciji) i prihvata stvarnih plaćanja. Posebno provjeriti: HR specifičnosti potrošačkog ugovora (ZZP, NN 41/14), pravo na otkazivanje pretplate (14-dnevno pravo na jednostrani raskid), porezne obaveze, jurisdikcijske klauzule.

---

## 1. Definicije

- **Aplikacija**: web servis "LEGAL-SUITE" dostupan na adresi na kojoj je objavljen te eventualnim budućim domenama.
...
> *Generirano iz LEGAL-SUITE · ID: NN-NNNN-NNNNNN*
...
Plaćanje se obavlja kroz **Polar.sh**, koji nastupa kao Merchant of Record. Davatelj **ne pohranjuje** podatke o kartici Korisnika; obradu kartice provodi pružatelj naplate. U kodu postoji i naslijeđena Stripe integracija (Stripe Payments Europe Ltd., PCI-DSS Level 1); ako je aktivirana, isto vrijedi za Stripe.

### 5.3 PDV

PDV se automatski izračunava i prikazuje pri checkout-u: Polar.sh kao Merchant of Record obračunava PDV za EU jurisdikcije (za Hrvatsku 25 %). Korisnik vidi konačnu cijenu prije potvrde plaćanja.
...
Refundi se odobravaju u skladu sa zakonom (čl. 5.4 i prema pravilima pružatelja naplate). Refund obrađuje pružatelj naplate (Polar.sh); vrijeme obrade može trajati do 10 radnih dana. Po refundu se PRO entitlement **odmah** ukida i Korisnik se vraća na besplatan tier.
...
- Privremene nedostupnosti Aplikacije (planiranog ili neplaniranog downtime-a, prirodnih nepogoda, kvarova trećih strana: Streamlit Cloud, Supabase, Cloudflare, Polar.sh, Stripe).
