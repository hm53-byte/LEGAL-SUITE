...
[Korisnik browser]
      │
      ▼
[Streamlit Community Cloud]  (postojeci, <adresa objavljene aplikacije>)
...
2. **Create organization**:
   - Name: `LEGAL-SUITE` (ili tvoja j.d.o.o. tvrtka)
   - Slug: `legal-suite` (bit će u URL-u checkout-a)
3. **Sandbox mode** (toggle u dashboardu) — radi sve test prije nego ideš live

4. **Products → Create new product**:
   - Name: `LEGAL-SUITE PRO`
...
3. **Pripremi worker direktorij**:
   ...
   ```bash
   cd "<lokalna kopija repozitorija>/cloud"
...
5. **Postavi secrets** (svaki je odvojena `wrangler secret put` komanda — wrangler će pitati vrijednost):
   ...
   wrangler secret put APP_RETURN_URL              # paste <adresa objavljene aplikacije>
   ```
...
1. **Otvori** `<adresa objavljene aplikacije>`
...
6. Refresh stranice
7. Generiraj bilo koji dokument → docx footer treba imati `ID: NN-NNNN-NNNNNN` (bez "Generirano iz LEGAL-SUITE" jer si PRO)
