...
def login_stranica():
    ...
    with col2:
                ...
                if submit:
                    if not email or not password:
                        ...
                        if email == admin_email:
                            # Bez postavljenog hasha admin prijava se odbija.
                            # Raniji kod je ovdje imao rezervnu lozinku u
                            # izvornom kodu, sto je u javnom repozitoriju
                            # znacilo da se svatko moze prijaviti kao admin
                            # dok god admin_password_hash nije postavljen.
                            ok = (
                                _verify_password(password, admin_pw_hash)
                                if admin_pw_hash
                                else False
                            )
                            if not admin_pw_hash:
                                st.error(
                                    "Administratorska prijava nije postavljena: "
                                    "nedostaje admin_password_hash u secrets."
                                )
                            if ok:
        ...
        st.markdown(
            ...
            "LEGAL-SUITE &copy; 2026 &middot; v4.1"
            "</div>",
            unsafe_allow_html=True,
        )
